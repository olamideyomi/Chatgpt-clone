...
const Wrapper = tw.div`h-screen w-screen flex flex-col`;
const Header = styled.header`
  ${tw`h-24 w-full bg-white shadow flex items-center justify-center text-2xl`}
`;
...
const Feed = ({ feed, loading, error }) => {
  ...
  return (
    <FeedWrapper>
      ...
      {error && <Message blueMsg>{error}</Message>}
    </FeedWrapper>
  );
};

const FeedWrapper = tw.div`flex-1 overflow-y-auto px-4 py-2 max-h-[calc(100vh - 96px - 60px)] flex flex-col gap-4`;
